...
- A pure function in `core/paths.ts` resolves `imageDestination` — variables, `..`, and the host-independent absolute check (`/x`, `C:\x` and `\\server\x` are rejected on every OS) — into a path relative to the document's folder. `settings.ts` supplies the document's location (its base name and its folder relative to the workspace folder), and the provider emits the AC6 warning, before the core's `convert()` is called.
